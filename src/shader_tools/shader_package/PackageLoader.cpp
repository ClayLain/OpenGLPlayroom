...
tl::expected<Type, Error> parseShaderType(const QString &string)
{
    ...
    if (it == stringToTypeMap.cend()) {
        return tl::unexpected { Error { "Can't parse shader type: " + string.toStdString() } };
    }
...
tl::expected<Package, Error> PackageLoader::load(const std::string &path)
{
    ...
    if(!m_archive.open(QuaZip::mdUnzip)) {
        return tl::unexpected { Error { "Can't open shader package file" } };
    }
...
tl::expected<QString, Error> PackageLoader::readFileContent(const QString &path)
{
    ...
    if (!manifestFileExist) {
        return tl::unexpected { Error { "Shader package file missed: " + path.toStdString() } };
    }
    ...
    if (manifestContent.isEmpty()) {
        return tl::unexpected { Error { "Can't read package file content: " + path.toStdString() } };
    }

    return manifestContent;
}

tl::expected<ShaderInfo, Error> PackageLoader::parseShaderInfo(const QJsonObject &object)
{
...
tl::expected<Package, Error> PackageLoader::parseManifest(const QString &manifestContent)
{
    auto parseError = QJsonParseError {};
    const auto manifestDocument = QJsonDocument::fromJson(manifestContent.toUtf8(), &parseError);
    if (parseError.error) {
        return tl::unexpected { Error { "Can't parse json: " + parseError.errorString().toStdString() } };
    }

    if (!manifestDocument.isObject()) {
        return tl::unexpected { Error { "Manifest root is not object" } };
    }
    ...
    if (!idOpt.has_value()) {
        return tl::unexpected { Error { "Invalid package id: " + idString } };
    }
    ...
    if (!versionOpt.has_value()) {
        return tl::unexpected { Error { "Invalid package version: " + versionString } };
    }

    auto shaders = std::vector<ShaderInfo> {};
    const auto shadersJsonValue = rootObject["shaders"];
    if (!shadersJsonValue.isArray()) {
        return tl::unexpected { Error { "Shaders value is not array" } };
    }
